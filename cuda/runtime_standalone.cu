...
const Tag MAT = 0x9; // numeric if-then-else
...
__device__ void subst(Unit* unit, Net* net, Book* book, Ptr a_ptr, Ptr a_dir, Ptr b_ptr, bool put) {
  Ptr* a_ref = target(net, a_dir);
  if (is_var(a_ptr)) {
    *target(net, a_ptr) = b_ptr;
  } else if (is_pri(a_ptr) && is_pri(b_ptr) && a_ptr < b_ptr || put) {
    put_redex(unit, b_ptr, a_ptr);
  }
  *a_ref = NONE;
}
