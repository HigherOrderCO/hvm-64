...
pub fn do_E(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  net.link_trg_ptr(rtx, Ptr::ERA);
  let (rtyx, rtyy) = net.do_ctr(rty, 0);
  net.link_trg_ptr(rtyx, Ptr::ERA);
  let (rtyyx, rtyyy) = net.do_ctr(rtyy, 0);
  net.link_trg(rtyyx, rtyyy);
}

pub fn do_I(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtyx, rtyy) = net.do_ctr(rty, 0);
  net.link_trg_ptr(rtyx, Ptr::ERA);
  ...
  net.link_trg_ptr(rtyyyx, Ptr::ERA);
  net.link_trg(rtyyxy, rtyyyy);
}

pub fn do_O(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtyx, rtyy) = net.do_ctr(rty, 0);
  ...
  let (rtyyx, rtyyy) = net.do_ctr(rtyy, 0);
  net.link_trg_ptr(rtyyx, Ptr::ERA);
  let (rtyyyx, rtyyyy) = net.do_ctr(rtyyy, 0);
  net.link_trg_ptr(rtyyyx, Ptr::ERA);
  net.link_trg(rtyxy, rtyyyy);
}

pub fn do_S(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtyx, rtyy) = net.do_ctr(rty, 0);
  let (rtyxx, rtyxy) = net.do_ctr(rtyx, 0);
...
pub fn do_Z(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  net.link_trg_ptr(rtx, Ptr::ERA);
  let (rtyx, rtyy) = net.do_ctr(rty, 0);
...
pub fn do_brn(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 0);
  net.link_trg_ptr(rtxx, Ptr::new_ref(&DEF_brnS));
  let (rtxyx, rtxyy) = net.do_ctr(rtxy, 0);
  net.link_trg_ptr(rtxyx, Ptr::new_ref(&DEF_brnZ));
  net.link_trg(rtxyy, rty);
}

pub fn do_brnS(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
...
pub fn do_brnZ(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  net.link_trg_ptr(rtx, Ptr::ERA);
  ...
  let (rd1x, rd1y) = net.do_ctr(rd1, 0);
  net.link_trg_ptr(rd1x, Ptr::new_ref(&DEF_I));
  let (rd1yx, rd1yy) = net.do_ctr(rd1y, 0);
  net.link_trg_ptr(rd1yx, Ptr::new_ref(&DEF_E));
  net.link_trg(rd0x, rd1yy);
}

pub fn do_c10(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
...
pub fn do_c12(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c14(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c16(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c20(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c22(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c24(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c3(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c4(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c6(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_c8(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 1);
  let (rtxxx, rtxxy) = net.do_ctr(rtxx, 1);
...
pub fn do_dec(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 0);
  net.link_trg_ptr(rtxx, Ptr::new_ref(&DEF_decO));
  let (rtxyx, rtxyy) = net.do_ctr(rtxy, 0);
  net.link_trg_ptr(rtxyx, Ptr::new_ref(&DEF_decI));
  let (rtxyyx, rtxyyy) = net.do_ctr(rtxyy, 0);
  net.link_trg_ptr(rtxyyx, Ptr::new_ref(&DEF_E));
  net.link_trg(rtxyyy, rty);
}

pub fn do_decI(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
...
pub fn do_decO(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
...
pub fn do_low(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 0);
  net.link_trg_ptr(rtxx, Ptr::new_ref(&DEF_lowO));
  let (rtxyx, rtxyy) = net.do_ctr(rtxy, 0);
  net.link_trg_ptr(rtxyx, Ptr::new_ref(&DEF_lowI));
  let (rtxyyx, rtxyyy) = net.do_ctr(rtxyy, 0);
  net.link_trg_ptr(rtxyyx, Ptr::new_ref(&DEF_E));
  net.link_trg(rtxyyy, rty);
}

pub fn do_lowI(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let rd0 = Trg::Ptr(Ptr::new_ref(&DEF_I));
  let (rd0x, rd0y) = net.do_ctr(rd0, 0);
...
pub fn do_lowO(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
...
pub fn do_main(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let rd0 = Trg::Ptr(Ptr::new_ref(&DEF_c6));
  let (rd0x, rd0y) = net.do_ctr(rd0, 0);
  net.link_trg_ptr(rd0x, Ptr::new_ref(&DEF_S));
  let (rd0yx, rd0yy) = net.do_ctr(rd0y, 0);
  net.link_trg_ptr(rd0yx, Ptr::new_ref(&DEF_Z));
  let rd1 = Trg::Ptr(Ptr::new_ref(&DEF_brn));
  let (rd1x, rd1y) = net.do_ctr(rd1, 0);
...
pub fn do_run(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let (rtxx, rtxy) = net.do_ctr(rtx, 0);
  net.link_trg_ptr(rtxx, Ptr::new_ref(&DEF_runO));
  let (rtxyx, rtxyy) = net.do_ctr(rtxy, 0);
  net.link_trg_ptr(rtxyx, Ptr::new_ref(&DEF_runI));
  let (rtxyyx, rtxyyy) = net.do_ctr(rtxyy, 0);
  net.link_trg_ptr(rtxyyx, Ptr::new_ref(&DEF_E));
  net.link_trg(rtxyyy, rty);
}

pub fn do_runI(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let rd0 = Trg::Ptr(Ptr::new_ref(&DEF_run));
  let (rd0x, rd0y) = net.do_ctr(rd0, 0);
...
pub fn do_runO(net: &mut Net, rt: Ptr) {
  let rt = Trg::Ptr(rt);
  let (rtx, rty) = net.do_ctr(rt, 0);
  let rd0 = Trg::Ptr(Ptr::new_ref(&DEF_run));
  let (rd0x, rd0y) = net.do_ctr(rd0, 0);
